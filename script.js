...
const diceEl = document.querySelector('.dice'); //#3 the dice image
...
let scores, currentScore, activePlayer, playing; // #35 Declare variables outside of the init function, otherwise we will get an error

const init = function () { //#33 Starting conditions turned into an init function
  scores = [0, 0]; //#32 resetting back to initial state
  currentScore = 0; //#32
  activePlayer = 0; //#32
  playing = true; //#32

  current0El.textContent = 0; //#32
  current1El.textContent = 0; //#32
  score0El.textContent = 0; //#32
  score1El.textContent = 0; //#32

  diceEl.classList.add('hidden'); //#32
  player0El.classList.remove('player--winner'); //#32
  player1El.classList.remove('player--winner'); //#32
  player0El.classList.add('player--active'); //#32
  player1El.classList.remove('player--active'); //#32
};

init(); //#34 call the init function

btnNew.addEventListener('click', init); //#36 call init function when clicking new game

const switchPlayer = function () {
  //#21 switch player making it a function to make it DRY since it will be used twice
...
btnRoll.addEventListener('click', function () {
  if (playing) {
    //#29 set if condition to only execute if it is true
...
btnHold.addEventListener('click', function () {
  //#19 holding the score for each player
  if (playing) {
    //#29 set if condition to only execute if it is true
    //1. Add current score to active player's score
    scores[activePlayer] += currentScore; //#20 Updating the score. The same as scores[1] = scores[1] + currentScore; //Example if it was player 2 (First item in the array). 
    document.getElementById(`score--${activePlayer}`).textContent = scores[activePlayer];

    //2. Check if player's score is >= 100 ..Finish the game
    if (scores[activePlayer] >= 10) {
      //#23 set condition that if the total score of the active player is equal or over 100 then..
      ...
      alert(`Player ${activePlayer + 1} wins`);//#36 Alert the winner
    } else {
      // #26 Add else to switch the player
